/**
 * ID Generator Service Implementation — Alertworth prefixes.
 */

import type { DomainCode } from '@alertworth/core/_shared/helpers';
import { DOMAIN_PREFIX_MAP, isValidDomainId } from '@alertworth/core';
import { ulid } from 'ulid';
import type { IdGeneratorService } from '@alertworth/services/_shared';
...
export class DefaultIdGeneratorService implements IdGeneratorService {
  ...
  evtId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.events);
  }
  scrId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.scoring);
  }
  trgId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.triage);
  }
  invId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.investigations);
  }
  bslId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.baselines);
  }
  ctlId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.controls);
  }
  indId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.indicators);
  }
  rptId(): string {
