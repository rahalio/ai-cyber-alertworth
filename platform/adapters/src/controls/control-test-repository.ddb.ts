...
import type { ControlTestRepository } from "@alertworth/services/controls";
...
export class ControlTestRepositoryDdb implements ControlTestRepository {
  ...
  async scheduleControlTest(input: Parameters<ControlTestRepository['scheduleControlTest']>[0]): Promise<Awaited<ReturnType<ControlTestRepository['scheduleControlTest']>>> {
    const inputExt = input as Record<string, unknown>;

    const idVal = inputExt.id;
