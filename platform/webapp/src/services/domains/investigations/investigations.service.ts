 ...
 *
 * API client for investigations domain.
...
const rawInvestigationsService = {
  /**
   * List investigation cases
   */
  async getInvestigation(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/investigations` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getInvestigation_2(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/investigations`;
   ...
   */
  async getDisposition(caseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/investigations/${caseId}/disposition`;
   ...
   */
  async getChallengeDismissal(caseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/investigations/${caseId}/challenge-dismissal`;
...
export const investigationsService = makeService(rawInvestigationsService, "investigations");
