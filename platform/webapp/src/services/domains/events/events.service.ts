...
const rawEventsService = {
  ...
  async getEvent(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/events` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getEvent_2(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/events`;
...
export const eventsService = makeService(rawEventsService, "events");
