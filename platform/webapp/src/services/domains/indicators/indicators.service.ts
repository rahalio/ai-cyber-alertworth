 ...
 * Indicators Service
 *
 * API client for indicators domain.
...
const rawIndicatorsService = {
  /**
   * List key risk indicators
   */
  async getKri(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/kris` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getKri_2(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/kris`;
   ...
   */
  async updateRefresh(kriId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/kris/${kriId}/refresh`;
...
export const indicatorsService = makeService(rawIndicatorsService, "indicators");
