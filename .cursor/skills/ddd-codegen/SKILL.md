...
Package scope: `@alertworth` (override via `package_scope` in config when cloning).

## HARD RULE — Never push codegen to GitHub

**Do not commit or push `.codegen/` (tooling, configs, generators, examples) to GitHub.**  
Keep it local-only. Agents must not `git add` / commit / push that tree. OpenAPI sources and generated product layers under `packages/` / `platform/` are separate; this rule is about the **codegen tool itself**.
...
2. Set `package_scope` (e.g. `@acme`) and rename `@alertworth/*` packages.
